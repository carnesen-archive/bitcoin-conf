...
type LoadConfigurationOptions = {
  conf?: string; // Absolute or datadir-relative path
  datadir?: string; // Absolute or relative path
};

const loadConfigurationSync = (options: LoadConfigurationOptions = {}) => {
  ...
  const loadedConfiguration = decode(confFileContents) as Configuration;
  return loadedConfiguration;
};

type LoadRpcHrefOptions = LoadConfigurationOptions;
export const loadRpcHrefSync = (options: LoadRpcHrefOptions = {}) => {
  const loadedConfiguration = loadConfigurationSync(options);
